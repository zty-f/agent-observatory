...
![Agent Observatory dashboard showing synthetic Codex, Claude Code, OpenClaw, and Pi sessions](docs/images/dashboard-demo.png)
...
Read the [Privacy Notice](docs/PRIVACY.en.md) ([中文](docs/PRIVACY.md)) and [Security Policy](.github/SECURITY.en.md) before sharing screenshots, logs, or access to the dashboard. **Never attach raw session files to a public issue.**
...
`npm test` checks JavaScript syntax and runs focused HTTP privacy/security tests. The root `server.js` remains the stable entry point for existing background jobs. The implementation lives in `src/`, and browser assets live in `public/`.

```text
agent-observatory/
├── README.md / README.en.md  # Chinese landing page and English edition
├── server.js                 # stable service entry point
├── start.sh / stop.sh        # macOS service commands
├── src/                     # session readers, actions, Codex state helper
├── public/                  # HTML, CSS, browser JavaScript
├── docs/                    # privacy notices and demo image
├── .github/                 # community files, templates, CI
└── test/                    # local HTTP security tests
```

## Contributing and support

Read [contribution guide](.github/CONTRIBUTING.en.md) and the [Code of Conduct](.github/CODE_OF_CONDUCT.md) before opening a pull request. Use the [issue templates](https://github.com/zty-f/agent-observatory/issues/new/choose) for bugs and ideas; remove personal paths and session content from examples. Report vulnerabilities privately as described in [security policy](.github/SECURITY.en.md).
